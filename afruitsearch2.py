...
class Operate:
    def __init__(self, args):
        ...
        # sandra: aux objects/vars added for m4
        self.baseline = 0
        self.scale = 0
...
############################## AUTO FRUIT SEARCH FUNCTIONS FOR OPERATE CLASS ########################
    ...
    #     self.d_star_lite = DStarLite(ox, oy)
    ...
    #         _, pathx, pathy = self.d_star_lite.main(Node(x=sx[i], y=sy[i]), Node(x=gx[i], y=gy[i]), spoofed_ox=[], spoofed_oy=[])
    #         pathx.pop(0)
    #         pathy.pop(0)
    ...
    # obtain a searching index and the corresponding true pos of the fruits for algorithm
        ...
        for i in range(len(search_list)):
            ...
            self.waypoints_list.append(astar.a_star_search(grid, grid_src, grid_dest))
            src = dest
            # Feedback
        print(f"Path generated: {self.waypoints_list}") 
    ...
    def drive_to_point(self, waypoint, canvas):
        ...
        #     turn_time = abs(turn_diff*(baseline*np.pi/scale/wheel_vel)/(2*np.pi))
    ...
    def robot_move_rotate(self,turning_angle=0, wheel_lin_speed=0.7, wheel_rot_speed=0.4):
        '''
        ...
        while elapsed < turning_time:
            self.pibot_control.set_velocity(wheel_rotation_speeds)
    ...
    def robot_move_straight(self, dist_to_waypt=0, wheel_lin_speed=0.7, wheel_rot_speed=0.4):
        '''
