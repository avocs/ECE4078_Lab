...
class Operate:
    def __init__(self, args):
        ...
        if args.yoloV8 == "":
            self.obj_detector = None
            self.yolo_vis = cv2.imread('ui/8bit/detector_splash.png')
        else:
            self.obj_detector = ObjectDetector(args.yoloV8)
            self.yolo_vis = np.ones((480,640,3))* 100
    ...
    def update_slam(self, drive_meas):
        ...
        if self.request_recover_robot:
            ...
            # TODO M3, disable updates to aruco markers if true map is loaded
    ...
    def record_data(self):
        # this saves slam map to slam.txt
        ...
        # upon pressing n, this is supposed to write into pred.txt
        if self.command['save_obj_detector']:
            if self.obj_detector_output is not None:            
                path_write = self.lab_output_dir
                    # def write_image(self, pred, state, pred_output_dir):  
                    # obj_detector_output = (bounding_boxes, robot_state)
                # ---- pred_fname is a string, fname is the filepath, returned due to cv2.imwrite conflicts
                ...
                self.pred_fname, fname = self.obj_detector.write_image(self.obj_detector_output[0], self.obj_detector_output[1], self.pred_output_dir)
                image = self.pibot_control.get_image()
                image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                f_ = os.path.join(self.pred_output_dir, f'pred_{self.image_id}.png')
    ...
    def detect_object(self):
        # upon pressing p, this saves the output image with the bbox labelled into raw_images, purely for visual inspection
        # the actual raw_images, img_n.png will be used again when running inference, bbox wont be used
        if self.command['run_obj_detector'] and self.obj_detector is not None:
            # obj_detector_pred = bounding_boxes, yolo_vis = output_img
            self.obj_detector_pred, self.yolo_vis = self.obj_detector.detect_single_image(self.img)
            str_preds = [str(pred) for pred in self.obj_detector_pred]
            f = os.path.join(self.raw_img_dir,f'bboximg_{self.checkimgid}.png')
            self.checkimgid += 1
            # TODO cvtColor called, bounding box labelled img being saved here for visual inspection
            self.cvtimg = cv2.cvtColor(self.yolo_vis, cv2.COLOR_RGB2BGR)
            cv2.imwrite(f,self.cvtimg)
    ...
    def save_image(self):
        if self.command['save_image']:
            image = self.pibot_control.get_image()
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            # TODO since the model runs inference on this set of pics, there is no need for greyscale, make sure obj_pose_est runs from this directory
